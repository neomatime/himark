...
/* ============================================================
   HOME HERO — post-loader cinematic entrance helpers
   ============================================================
   Two small jobs that the CSS choreography in
   styles/redesign.css can't do on its own:

   1. WORD SPLIT for the paragraph typewriter — wraps each word
      of .heroe-sub in <span class="hero-word" style="--i:N">.
      The CSS reads --i to stagger each word's reveal via
      animation-delay: calc(1.4s + (var(--i) * .045s)).

   2. BG-ENTERING CLASS LIFECYCLE — adds .bg-entering to the
      hero on load (this triggers the heroBgEntrance keyframes
      that handle scale + blur over 2.6s starting at +4.6s),
      then removes it when the animation ends. The static rule
      `.hero-editorial.full::before { transform: scale(var
      (--hero-zoom)); filter: blur(10px) }` ends at the SAME
      final values, so the swap is invisible — but it's
      necessary because, after the swap, the scroll-zoom
      handler that updates --hero-zoom on scroll can drive the
      transform again. Leaving the animation in place would
      override the scroll-zoom.

   Runs once per page load. Respects prefers-reduced-motion.
   Self-contained — no-ops on every page except home.
   ============================================================ */
(function homeHeroEntrance(){
  ...
  function boot(){
    var hero = document.querySelector('.hero-editorial.full');
    if (!hero) return;

    /* ---- 1. Split the sub-paragraph into per-word spans ---- */
    var sub = hero.querySelector('.heroe-sub');
    if (sub && !reducedMotion && !sub.dataset.split){
      sub.dataset.split = '1';
      /* Preserve any HTML the paragraph already carries (em /
         strong / spans). Walk text nodes only, wrap each word
         in a <span>, leave other inline elements alone. */
      var walker = document.createTreeWalker(sub, NodeFilter.SHOW_TEXT, null);
      var textNodes = [];
      var n;
      ...
      textNodes.forEach(function(node){
        ...
        if (parts.length === 1 && !parts[0].trim()) return;  /* pure whitespace */
    ...
    /* ---- 2. Background entrance class — triggers the
       heroBgEntrance keyframes from styles/redesign.css ---- */
    if (!reducedMotion){
      hero.classList.add('bg-entering');
      hero.addEventListener('animationend', function onAnim(e){
        if (e.animationName === 'heroBgEntrance'){
          /* Drop the class so the static rule (which uses
             var(--hero-zoom)) takes over — scroll-zoom will
             then drive the transform as the user scrolls. */
          hero.classList.remove('bg-entering');
          hero.removeEventListener('animationend', onAnim);
        }
      });
    }
  }
